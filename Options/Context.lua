...
BR.Options.Groups = {
    ...
    },
    {
        ...
        -- categories follow the All Buffs page reading order (left column
        -- top-to-bottom, then right column) so the sidebar mirrors how users
        -- scanned the buffs there. Custom sits last because it's qualitatively
        -- different (user-defined entries with a list editor).
        id = "displayBehavior",
        titleKey = "Sidebar.DisplayBehavior",
        pages = { "defaults", "raid", "targeted", "consumable", "presence", "self", "pet", "custom" },
    },
    {
...
-- 14x14 pin button that toggles whether a buff icon is detached from its
-- category frame. Used in the All Buffs row factory and the Custom Buffs page
-- — same visual + click + tooltip behavior in both places.
function Helpers.CreateDetachPin(parent, key)
    ...
    local function UpdateVisual()
        if IsIconDetached(key) then
            icon:SetVertexColor(1, 0.85, 0.3, 1)
